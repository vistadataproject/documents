...
Each of the original CPRS RPCs will be incrementally audited, emulated, isolated, and secured by the __RPC Emulator__, with all semantics reflected in the Master VistA Data Model (__MVDM__). The RPC Emulator audits and prevents any code injection, and directs all database access correctly through the Fileman API (database management system). 
