...
__Data Linkage__  | __MVDM enables VISTA to natively and dynamically link its data to thousands of external Linked Data resources on the web, dramatically enriching VISTA's data.__ Linked Data is used by the largest web search engines (including Google, Yahoo, and Microsoft) to index, search and link to all the structured data on the web. By sharing the same Linked Data model and technology with the world's largest web search organizations, VISTA inherits the capability to link to all these structured data sources as well.  Linking specific patient information from VISTA, for example, would allow highly personalized enrichment of the veteran information with always-up-to-date authoritative sources of information.
