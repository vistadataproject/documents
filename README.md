...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, securely, and semantically link VISTA data directly with all external Linked Data resources.__  The worlds largest web search companies (Google, Yahoo, Microsoft, and others) have all converged on Linked Data as the medium for publication, indexing, and semantically searching all structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the capability to semantically link and mesh its data with all these structured web data sources as well.  Linking patient information in VISTA, for example, would allow highly personalized enrichment of the patient's record with always-up-to-date authoritative sources of external information.
