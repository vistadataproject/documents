...
__Web-Scale__  | The VISTA Data Model uses the same universal world wide web Linked Data model and technology used by Google, Yahoo, Microsoft, and other major search engines to crawl, link, and index *all structured data on the web.* Leveraging this common web-scale Linked Information Space would allow VISTA to dynamically link with thousands of Linked Data resources on the web, creating a highly enriched, personalized, precision health record that is connected to always-up-to-date best evidence.
