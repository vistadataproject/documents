...
__Data Integrity__ | Provides VISTA with its first single, comprehensive framework for strict validation and consistency checks for all information provided to or from all clients. *This is a major improvement over the hodge-podge of methods (HL7, RPCs, procedural code), which are all inconsistent, unpredictable, and permissive*
__Data Security__ | Provides VISTA with its first comprehensive, granular, on-the-data, *Data-Level Security Model*. Currently VISTA is data-category agnostic  By metadata enrichment of the VDM with data categories, VISTA will know *what categories of data it is managing*.  This is a form of *semantic security* which is the highest level of security possible.  This is the foundation for any other security measures that one needs to take, including encryption, obfuscation, etc. since without knowing what the data is, one won't know what level of security to apply to it.  See [Data-Centric Security](https://en.wikipedia.org/wiki/Data-centric_security)
