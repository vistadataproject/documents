...
* By virtue of VA's formal technical review and approval of the Resource Description Framework (RDF) in the OneVA Technical Reference Model, VA  endorses the RDF (web-standards-based data model) for the VA enterprise architecture. JSON-LD is one of many serializations of RDF. See [TRM-RDF](http://www.va.gov/TRM/StandardPage.asp?tid=6405) and [JSON-LD](http://json-ld.org).
