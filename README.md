...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, securely, and semantically link VISTA data directly with all external Linked Data resources, dramatically enriching VISTA's data.__  The worlds largest web search companies (Google, Yahoo, Microsoft, and others) have all converged on Linked Data as the medium for publication, indexing, and semantically searching all structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the capability to semantically link and mesh its data with all these structured web data sources as well.  Linking patient-specific information in VISTA, for example,  would allow highly personalized enrichment of their health record with always-up-to-date authoritative sources of external information.
__Representative__  | __MVDM operationalizes all relevant VA VISTA data to the maximum extent available.__ <br> The VISTA Data Model comprises the current existing data-driven architecture of VISTA, and thus leverages all existing VISTA definitions. There is 100% coverage of the data definitions of any local VISTA and that of its corresponding  VISTA Data Model (VDM), and these are maintained always-up-to-date. Any and all enhancements to the VISTA system and its data definitions will automatically be reflected in the VISTA Data Model through automated, triggered updates whenever VISTA's data dictionary is updated. 
