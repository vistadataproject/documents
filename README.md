...
__Data-Centric__ | __MVDM is a new, purely data-centric approach to managing VISTA's data__.  
It does not involve changing a single line of the 2.2 million lines of VISTA's existing M procedural code, nor is it 'wrapping' (i.e. secretly using) any legacy code, routines, or RPCs dressed up as some new programming language, API, or encapsulation container. Rather, this surfaces the data itssself and the data model - which up to this point has never been done. *This is the opposite of a code-centric approach, which obfuscates the data*.
