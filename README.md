...
__Self-Contained__ | __The  VISTA Data Model runs entirely server-side, embedded directly on the existing VISTA database__. <br> This eliminates all moving parts and maximizes transaction processing performance by running as an embedded process directly on the local database, leveraging the 'as-is' database architecture. *This makes it easy to deploy, maintain, and keep highly performant.  No moving parts. No external dependencies. No middleware.* 
__Real-Time__ | __The VISTA Data Model is operationalized using Best-of-Breed real-time server-side runtime technology.__<br> This same runtime technology drives the largest commericial transactional websites such as PayPal, LinkedIn, New York Times, Netflix, and Uber. *This maximizes transactional processing performance directly on the transactional database.*
__Web-Scale__  | __The VISTA Data Model uses the same web-standard, web-scale Linked Data model and technology that Google, Yahoo, Microsoft, and other major search engines use to crawl, link, and index all structured data on the web.__ <br> Leveraging this common Linked Data medium allows VISTA to become a first-class citizen of the Internet of Data, enabling VISTA to dynamically link to thousands of Linked Data resources on the web. Semantic, dynamic linking of information from VISTA would allow, for example, the enrichment of the health record with specific, always-up-to-date information, personalized to each patient.
