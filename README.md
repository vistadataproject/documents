...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, semantically link VISTA data with external structured web data resources.__  The major world wide web search companies (Google, Yahoo, Microsoft, and others) and knowledge orgainizations (including the National Library of Congress and National Library of Medicine) have converged on the Linked Data standard as the medium they use for publication, indexing, and semantically searching, and linking to the vast amount of structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the same capability to semantically link its data with all these external structured web data sources.  Linking a patient's information from VISTA to external authoritative healthcare and lifesciences sources, for example,  will create a highly contextualized, patient-specific enrichment of their health record.
