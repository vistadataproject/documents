...
__Data Security__ | __MVDM provides VISTA with its first comprehensive, fine-grained, data-centric *Data Security Model*.__ <br> Currently VISTA is data-category agnostic. Through metadata enrichment of the VISTA Data Model, VISTA will know for the first time *what categories of data it is managing* (patient data, institution data, etc.). Data-centric, semantic (logical) security is the foundation for all other security levels and technologies, because without knowledge of what the data is or its category, it will not be possible to provide the appropriate security measures to the data.  See [Data-Centric Security](https://en.wikipedia.org/wiki/Data-centric_security),  [Logical Security](http://www.mdpi.com/1999-5903/4/4/929/htm#fig_body_display_futureinternet-04-00929-f001), and [Semantic Security](https://www.google.com/search?q=semantic+data+security&sa=X&biw=1154&bih=1062&tbm=isch&tbo=u&source=univ&ved=0ahUKEwi_14b--JXLAhWKOz4KHWghAVEQsAQIgwE)
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, securely, and semantically link VISTA data directly with all external Linked Data resources, dramatically enriching VISTA's data.__  The worlds largest web search companies (Google, Yahoo, Microsoft, and others) have all converged on Linked Data as the medium for publication, indexing, and searching all structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA shares this capability to semantically link to all available web data sources in structured, computable form.  Linking patient-specific information in VISTA, for example,  would allow highly personalized enrichment of their health record with always-up-to-date authoritative sources of external information.
