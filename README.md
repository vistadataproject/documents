...
The VISTA Data Project is a new data-centric approach to comprehensively exposing, representing, and managing all 35 years of patient data and institutional know-how currently in the VA's 131 nationally deployed health information systems (VISTA) using a modern web-standard data model; and by enriching and operationalizing this model, make all relevant VISTA data securely accessible and computable in real-time across all 131 VISTA systems as one national, standard Master VISTA system.
...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, semantically link VISTA data with all external Linked Data resources.__  All major world wide web search engies (Google, Yahoo, Microsoft, and others) and knowledge orgainizations (National Library of Congress, National Library of Medicine) have converged on the Linked Data standard as the medium they use for publication, indexing, and semantically searching the vast amount of structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the same capability to semantically link its data with all these external structured web data sources.  Linking a patient's information in VISTA to external authoritative healthcare and lifesciences sources, for example,  will create a highly contextualized, patient-specific enrichment of their health record.
...
__Data-Centric__ | __MVDM is a new, purely data-centric approach to managing VISTA's data__.  It does not involve changing a single line of the 2.2 million lines of VISTA's existing M procedural code, nor is it 'wrapping' (i.e. secretly using) any legacy code, routines, or RPCs dressed up as a shiny new programming language, API, or encapsulation container. Rather, this comprehensively surfaces all VISTA data - and exposes the fact that __VISTA has a data model__ - which up to this point has not been realized nor taken advantage of. *This is the opposite of a code-centric approach, which obfuscates the data*.
