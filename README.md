...
__Data Integrity__ | Provides VISTA with its first single, comprehensive framework for strict validation and consistency checks for all information provided to or from all clients. *This is a major improvement over the hodge-podge of legacy methods that have been used over the past 35 years (HL7, RPCs, procedural code), which are all inconsistent, unpredictable, and permissive*
__Data Security__ | Provides VISTA with its first comprehensive, granular, on-the-data, *Data-Level Security Model*. Currently VISTA is data-category agnostic. Through metadata enrichment of the VISTA Data Model with data categories, VISTA will know for the first time *what categories of data it is managing* (for example, patient data vs non-patient data).  This is the foundation for all other security measures that one needs to take (encryption, obfuscation, etc.) since without knowing what the data is, one won't know what level of security to apply to it.  This is a form of *semantic security* (i.e. knowledge of what the data is and means as a pre-requisite to applying security measures). See [Data-Centric Security](https://en.wikipedia.org/wiki/Data-centric_security)
