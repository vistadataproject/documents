...
__Data Security__ | Provides VISTA with its first comprehensive, granular, on-the-data, *Data-Level Security Model*. Currently VISTA is data-category agnostic. Through metadata enrichment of the VISTA Data Model with data categories, VISTA will know for the first time *what categories of data it is managing* (for example, patient data vs non-patient data). This is a form of *semantic security* (i.e. knowledge of what the data is and means as a pre-requisite to applying security measures). Data-Layer Sementic Security is the foundation for all other security measures (encryption, obfuscation, etc.), since without knowing what the data is, one won't know what kind or level of security to apply to it.  See [Data-Centric Security](https://en.wikipedia.org/wiki/Data-centric_security)
