...
![db-integrity](https://github.com/vistadataproject/documents/blob/master/images/logos/logos-db/50h/db-integrity.jpg) <br> __Integrity__| __Comprehensive, automated, standardized, strict data integrity enforcement for  *all VISTA data*.__ <br> *This is a major improvement over the hodgepodge of legacy, ad-hoc methods that have accumulated over the past 35 years (HL7, RPCs, MUMPS, procedural code), none of which are documented, and all of which are inconsistent, unpredictable, and highly permissive*
![db-security](https://github.com/vistadataproject/documents/blob/master/images/logos/logos-db/50h/db-security.jpg) <br> __Security__ | __Comprehensive, industry-standard, fine-grained, data-centric security for *all VISTA data*.__ <br> Currently VISTA provides security for only a small subset of its data through a roll-and-scroll interface menus, and does this through highly nonstandard, complex, and unmaintainable methods. Data-centric, attribute-based security is the foundation for all other security levels and technologies, because without knowledge of the data and its attributes, it will not be possible to provide the appropriate security measures on the data. Through metadata enrichment of the VISTA Data Model, VISTA will know for the first time *what categories of data it is managing* and thus allow, for the first time, data-centric, attribute-based security  "on-the-data" security. Providing granular security is a pre-requisite for allowing the exchange of any data within the enterprise and with any external systems. See [Data-Centric Security](https://en.wikipedia.org/wiki/Data-centric_security),  [Logical Security](http://www.mdpi.com/1999-5903/4/4/929/htm#fig_body_display_futureinternet-04-00929-f001), [Semantic Security](https://www.google.com/search?q=semantic+data+security&sa=X&biw=1154&bih=1062&tbm=isch&tbo=u&source=univ&ved=0ahUKEwi_14b--JXLAhWKOz4KHWghAVEQsAQIgwE) and [Attribute-Based Access Control (ABAC)](http://csrc.nist.gov/projects/abac)
![db-linkage](https://github.com/vistadataproject/documents/blob/master/images/logos/logos-db/50h/db-linkage.jpg) <br> __Linkage__ | __Capability of direct and semantic linkage of *any VISTA data* to external structured data sources.__  The largest world wide web search companies (Google, Yahoo, Microsoft, and others) and knowledge orgainizations (including the National Library of Congress and National Library of Medicine) all use the Linked Data standard as the medium for publication, indexing, linking, and semantic search of the vast amount of structured data in their oranization and on the web. Because MVDM also uses the Linked Data model and technology, VISTA will also acquire the same capability to semantically link its data to all these data sources and to the vast amount of structured data on the web. Linking a patient's health information in VISTA to external authoritative healthcare data sources, for example,  will create a highly contextualized patient-specific enrichment of their health record.
