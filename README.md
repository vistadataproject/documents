...
__Data Linkage__  | __MVDM provides VISTA with capability to natively, securely, and semantically link its data directly with all Linked Data resources, dramatically enriching VISTA's data.__  The worlds largest web search companies (Google, Yahoo, Microsoft, and others) have all converged on Linked Data as the medium for publication, indexing, and searching all structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA shares this capability to semantically link to the same, vast external web data sources in structured, computable form.  Linking patient information in VISTA would allow personalized enrichment of the patient information with an unlimited range of always-up-to-date authoritative sources of external information.
