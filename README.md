...
Now, for the first time, VISTA's true, native, transactional data model - the **VISTA Data Model** - will be comprehensively exposed, enriched, and operationalized as a single, secure, symmetric (read-write) interface to all VISTA data.  The VISTA Data Model is in turn normalized across all local VISTA system models creating a national, enterprise __Master VISTA Data Model (MVDM__), which allows read-write transactions across any and all of the 100+ VISTA systems using a single, standard, secure mechanism.
