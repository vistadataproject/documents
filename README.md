...
__Web-Scale__  | __MVDM uses the same web-standard, web-scale Linked Data model and technology that all the largest web search engines use to crawl, link, and index all structured data on the web.__ <br> Leveraging this common Linked Data medium allows VISTA to become a first-class citizen of the Internet of Data, along with Google, Microsoft, and Yahoo,  enabling VISTA to dynamically link to thousands of Linked Data resources on the web. Semantic, dynamic linking of information from VISTA would allow, for example, highly personalized, precision enrichment of the veteran health record with always-up-to-date information.
