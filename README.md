...
__Web-Scale__  | __MVDM enabling VISTA to natively and dynamically link to thousands of external Linked Data resources on the web, making VISTA a first-class citizen of the Web of Data.__ This is because MVDM uses the same web-standard, web-scale Linked Data model and technology that the world's largest web search engines use to search and link to all structured data on the web. Linking specific patient information, for example, would allow highly personalized, precision enrichment of the veteran health record with information from external, always-up-to-date authoritative sources.
