...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, semantically link VISTA data with all external Linked Data resources.__  All major world wide web search companies (Google, Yahoo, Microsoft, and others) have converged on the Linked Data standard as the medium for publication, indexing, and semantically searching structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the same capability to semantically link its data with all these external structured web data sources.  Linking a patient's information to external authoritative healthcare and lifesciences information (such as all published journals in the the National Library of Medicine) will create highly personalized, contextualized, patient-centric enrichment of their health information with always-up-to-date evidence-based information to support improved clinical decisionmaking.
