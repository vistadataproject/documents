...
__Data Linkage__  | __MVDM provides VISTA with the capability to directly, securely, and semantically link VISTA data directly with all external Linked Data resources.__  The world wide web search companies (Google, Yahoo, Microsoft, and others) have all converged on the Linked Data standard as the medium for publication, indexing, and semantically searching structured data on the web. Because MVDM uses the same Linked Data model and technology, VISTA will also have the same capability to semantically link and mesh its data with all these structured web data sources.  Linking a patient's information to external always-up-to-date authoritative healthcare and lifesciences information, for example, will provide highly personalized, contextualized, patient-centric enrichment of their health information, and allow for much enhanced clinical decisionmaking.
