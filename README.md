...
__Data Linkage__  | __MVDM provides VISTA with capability to natively, securely link its data directly with the entire Web of Data, dramatically enriching VISTA's data.__  The worlds largest web search companies (including Google, Yahoo, and Microsoft) all have converged on Linked Data as the medium that they discover, index, structure, publish, and search data on the web in computable form. Because MVDM uses the same Linked Data model and technology, VISTA inherits the capability to securely link to a vast number of external data sources in structured, computable form.  Linking patient information in VISTA would allow highly personalized enrichment of the patient information with a vast array of always-up-to-date authoritative sources of information.
...
__Data-driven__ | __MVDM is a new, purely data-centric, data-driven approach to managing VISTA's data__.  It does not involve hacking a single line of the 2.2 million lines of VISTA's MUMPS procedural code. In other words, this is *the opposite of a code-centric approach*.
...
__Empiric Evolution__ | __Employs a new approach to emprically evolving VISTA's capabilities through rapid, iterative, functional prototypes.__ This allows the focus to remain on exploration of new techniques and approaches, rather than on more superficial end-user requirements. This is *the opposite waterfall development*.  See [spiral model](https://en.wikipedia.org/wiki/Spiral_model)
