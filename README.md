...
> VA therefore needs a __single, consistent mechanism for real-time  read-write transactions to all of the 130 local, unique VISTA systems as if they were only one, national master VISTA system, using web-standards technologies__.  This reduces the complexity of development and deployment of new transactional web data interfaces and services from 130 distinct VISTA systems to that of only one standardized computable master VISTA data model.
