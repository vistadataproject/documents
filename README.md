...
__Data Linkage__  | __MVDM provides VISTA with capability to natively, securely, and semantically link its data directly with all Linked Data resources, dramatically enriching VISTA's data.__  The worlds largest web search companies (including Google, Yahoo, and Microsoft) have all converged on Linked Data as the medium they discover, index, structure, publish, and search all data on the web in computable form. Because MVDM uses the same Linked Data model and technology, VISTA shares this capability to semantically link to the same, vast external web data sources in structured, computable form.  Linking patient information in VISTA would allow highly personalized enrichment of the patient information with virtually unlimited, always-up-to-date authoritative sources of external information.
