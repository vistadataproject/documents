...
* By virtue of VA's formal technical review and approval of the Resource Description Framework (RDF) in the OneVA Technical Reference Model, VA formally endorses use of RDF (web-standard data model) for use in the VA enterprise architecture. JSON-LD is only one of many serializations of RDF. See [TRM-RDF](http://www.va.gov/TRM/StandardPage.asp?tid=6405) and [JSON-LD](http://json-ld.org).
